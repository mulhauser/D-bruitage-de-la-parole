...
public class SoundSignal {
     ...
     *
     * @param newsignal   (short) contenant un signal
     * @param samplingFrequency d'echantillonnage
     */
	...
	*/
    ...
    public double[] recuperationtionSignal(int m, int N) {

        double[] res = new double[signal.length];
        int j = 0;
        // On récupère le signal tous les 8ms par morceau de 32ms
        double[] hamming = fenetrageHamming(N);
        double[] fenetrage;
        double[] sommeHamming = new double[signal.length];

        for (int i = 0; i < this.getSignalLength() - N; i += m) {


            // extraction instant i, signal 32ms avec hamming
            fenetrage = signal32MS(i, N);
        ...
        for(int i  = 0; i < signal.length; i++){
            double test = res[i]/sommeHamming[i];
            if(Double.isNaN(test)){
                res[i] = 0;
            }else {
                res[i] /= sommeHamming[i];
            }
        }
        return res;
    }
    ...
    public double[] signal32MS(int inst, int N) {
        double[] res = new double[N];
        double[] hamming = fenetrageHamming(N);
        for (int i = 0; i < res.length; i++) {
            res[i] = signal[inst + i] * hamming[i];
        }
        return res;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Fenetre de Hamming
            double[] fenetre = signal.recuperationtionSignal(8 * signal.getSamplingFrequency() / 1000, 32 * signal.getSamplingFrequency() / 1000);

            /*int i;
            for (i = 0; i < fenetre.length; i++) {
                System.out.println("signal " + i + " : " + fenetre[i]+" signal base : "+signal.getSignal()[i]);
            }*/

            short[] signalHamming = new short[fenetre.length];
            for(int i = 0; i < signalHamming.length; i++){
                signalHamming[i] = (short)fenetre[i];
            }
            SoundSignal signal3 = new SoundSignal();
            signal3.setSignal(signalHamming, signal.getSamplingFrequency());
            signal3.exportSignal("essai_hamming.wav", true);
            System.out.println("File created");
		/*for(int i = 0; i < hamming.length){
